...
from functions import people_centered_response
...
class AdaptationModel(Model):
    ...
    def __init__(self, 
        ...
        self.average_appraisal = self.avg_tot_appraisal

        # Data collection setup to collect data
        model_metrics = {
                        "total_adapted_households": self.total_adapted_households,
                        # "avg_risk_appraisal": self.avg_risk_appraisal,
                        # "avg_coping_appraisal": self.avg_coping_appraisal,
                        "avg_tot_appraisal": self.avg_tot_appraisal
                        # ... other reporters ...
                        }
        
        agent_metrics = {
                        # let op dat het alleen bij de households moet werken, niet ook bij gov agents, lambda, of ook toevoegen aan gov agent
    ...
    def avg_tot_appraisal(self):
        avg_risk = sum([agent.risk_appraisal for agent in self.schedule.agents if isinstance(agent,Households)])/self.number_of_households
        avg_coping = sum([agent.coping_appraisal for agent in self.schedule.agents if isinstance(agent,Households)])/self.number_of_households
        avg_tot = 0.5*(avg_risk + avg_coping)
        return avg_tot
    ...
    def step(self):
        ...
        if self.schedule.steps == 5:
            for agent in self.schedule.agents:
                ...
                agent.flood_damage_actual = calculate_basic_flood_damage(agent.flood_depth_actual, agent.is_adapted)
                
                # generate flood_response
                # depending on the flood_damage_actual households will adapt more or less, with some random variation
                flood_response = agent.flood_damage_actual*(1 + random.random())*0.1
                agent.coping_appraisal += flood_response
                agent.risk_appraisal += flood_response
        
        """
        choose appropriate government action
        action defined in functions for people_centered(people centered communication strategy for social media)
        """
        # this is the top_down-RC = conventional communication strategy (flyers) with smaller effect than social media        
        # we assume that flyers have only limited effect for already informed people
        if self.avg_tot_appraisal() < 0.2 and self.top_down == True:
            for agent in self.schedule.agents:
                increase = 0.03 #average increase
                agent.coping_appraisal += increase * (agent.flood_damage_estimated/0.5) * (random.random()/0.5)
                agent.risk_appraisal += increase * (agent.flood_damage_estimated/0.5) * (random.random()/0.5)
        # people-centered communication strategy for social media with more specific and higher effect than flyers
        # even for that communication strategy limits of effectiveness are implemented, representing very informed people not getting new information from campaigns
        elif self.avg_tot_appraisal() < 0.5 and self.people_centered == True:
            for agent in self.schedule.agents:
                increase = 0.03
                agent.coping_appraisal += people_centered_response(agent.coping_appraisal,agent.risk_appraisal, increase, agent.flood_damage_estimated)[0]
                agent.risk_appraisal += people_centered_response(agent.coping_appraisal,agent.risk_appraisal, increase, agent.flood_damage_estimated)[1]

        # we assume that from an appraisal of 0,5 commun communication manners have no effect any more           

        # Collect data and advance the model by one step
        self.datacollector.collect(self)
        # print(self.datacollector.collect)
        self.schedule.step()
